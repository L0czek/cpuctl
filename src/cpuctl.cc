...
const std::unordered_map<std::string, int> options = { { "offset.cpu_core", CPU_CORE_PLANE },
...
void print_all()
{
    ...
    for (const auto &option : options) {
        const auto &[name, plane] = option;
        std::cout << name << std::setw(25 - name.length()) << " => " << convert_offset_to_voltage(off.values[plane])
                  << "mv\n";
    }
}
